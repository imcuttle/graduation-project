...
//  Nan::HandleScope scope;
  ...
  if(info.Length() == 2 && info[0]->IsInt32() && info[1]->IsInt32() ) {
    ...
    neighbors = info[1]->IntegerValue();
  } else if(info.Length() == 1 && info[0]->IsInt32()) {
    r = info[0]->IntegerValue();
  }

  cv::Mat img = self->mat;
  v8::Local<v8::Value> constructorArgs[3] = {Nan::New<v8::Integer>(img.rows-2*r), Nan::New<v8::Integer>(img.cols-2*r), Nan::New<v8::Integer>(CV_8UC1)};

  Local<Object> img_to_return = Nan::New(Matrix::constructor)->GetFunction()->NewInstance(3, constructorArgs);
  Matrix *imgJs = Nan::ObjectWrap::Unwrap<Matrix>(img_to_return);

  circle_lbp(self->mat, imgJs->mat, r, neighbors);
...
NAN_METHOD(Matrix::RectLBP) {
  ...
  cv::Mat img = self->mat;
  v8::Local<v8::Value> constructorArgs[3] = {Nan::New<v8::Integer>(img.rows-2*1), Nan::New<v8::Integer>(img.cols-2*1), Nan::New<v8::Integer>(CV_8UC1)};

  Local<Object> img_to_return = Nan::New(Matrix::constructor)->GetFunction()->NewInstance(3, constructorArgs);
  Matrix *imgJs = Nan::ObjectWrap::Unwrap<Matrix>(img_to_return);

  rect_lbp(self->mat, imgJs->mat);
